...
private_train (비공개)              train-reservation (공개)
  앱 소스 · 빌드                      결과물만
  발급 스크립트 · 서명 · 워크플로  ──▶  licenses/ · license-policy.json
  판단은 전부 여기서                    revoked.json · Releases
        ▲                              docs/ (현황판)
        └──── repository_dispatch ───  relay.yml (요청 중계)
```

- **판단·서명·발급은 전부 비공개에서** 합니다. 개인키도, 정책 결정도 여기 있습니다.
- 공개 저장소에는 **앱이 받아가야 하는 파일**과 **사용자가 요청을 넣을 이슈**만 둡니다.
  앱은 자격증명이 없으므로 이 셋은 공개일 수밖에 없습니다.
- 공개 쪽 워크플로는 `relay.yml`(요청 중계)과 `pages.yml`(현황판 배포) 둘뿐입니다.

**이 문서의 명령은 비공개 저장소에서 실행합니다.** 산출물을 쓸 자리를
`LICENSE_DIST_DIR` 로 알려줘야 합니다.

```bash
git clone https://github.com/ohjn96/train-reservation.git dist
export LICENSE_DIST_DIR=dist
```

바꾼 뒤에는 `dist/` 안에서 커밋·푸시하면 적용됩니다.
...
- **공개키** → `app/licensing/public_key.py` 에 자동으로 기록됩니다. 커밋하세요.
  앱이 이걸로 검증합니다. 공개 저장소에는 공개키를 둘 필요가 없습니다.
