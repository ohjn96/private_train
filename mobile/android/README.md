...
```
앱 실행
 └ MainActivity ─ WebView ──────────► http://127.0.0.1:<무작위 포트>  (폰 안)
...
- **보안**: 서버는 127.0.0.1 의 무작위 포트에만 뜨고(파이썬이 `Bridge.onServerReady` 로 알려줌),
  설치마다 만든 무작위 토큰을 WebView 쿠키로만 넘깁니다. 토큰을 싣기 전에 `/__hello?nonce=` 로
  그 포트의 서버가 토큰을 아는지(HMAC-SHA256) 확인합니다 (`LocalServer.kt`). 같은 폰의 다른 앱이
  붙으면 403. WebView 는 그 포트 밖의 127.0.0.1 주소로 이동하지 않습니다.
  코레일 비밀번호·카드는 앱 메모리와 Keystore 로 암호화한 작업 파일에만 두고, 백업·기기 이전에서
  빠집니다(`data_extraction_rules.xml`). 화면은 FLAG_SECURE, 잠금 화면 알림은 내용을 가립니다.
