...
```
앱 실행 (BeeWare Toga)
 └ app.py ─ toga.WebView ───────────► http://127.0.0.1:17650  (폰 안)
 └ ServerHost (파이썬 스레드)
...
| `src/trainreservation/app.py` | 앱 입구. 서버를 띄우고, 뜨면 WebView 로 `/__auth?t=토큰` 을 연다 |
...
- **보안**: 서버는 127.0.0.1 에만 뜨고, 설치마다 만든 무작위 토큰을 첫 주소 `/__auth?t=…` 로 WebView 쿠키에 넘깁니다. 같은 폰의 다른 앱이 붙으면 403.
...
3. `briefcase build iOS` (시뮬레이터) → **스모크 테스트**: 시뮬레이터에 설치·실행하고, 러너에서
   `curl http://127.0.0.1:17650/login` 이 **403** 인지 확인 (파이썬 서버가 떴고 토큰 검사가 켜져 있음).
...
- **소켓 회수**: iOS 는 멈춰 있던 앱의 듣는 소켓을 회수할 수 있습니다 (Apple TN2277).
  앱이 다시 앞으로 오면 `/__health` 로 확인하고, 응답이 없으면 같은 Flask 앱으로 소켓만 다시 열고 WebView 를 새로 고칩니다.
