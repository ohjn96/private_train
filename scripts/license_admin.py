...
개인키를 다루므로 이 스크립트는 당신 PC 와 이 저장소의 Actions 에서만 돈다.
개인키는 저장소 바깥(기본: ~/.config/private_train/license_signing_key.pem)에
두며, 절대 커밋하지 않는다.

**산출물은 이 저장소가 아니라 공개 배포 저장소에 쓴다.** 앱이 자격증명 없이
받아가야 하기 때문이다. 공개 저장소를 체크아웃한 자리를 LICENSE_DIST_DIR 로
알려준다 (기본값은 ./dist).
...
# 산출물이 놓이는 곳 — 공개 배포 저장소를 체크아웃한 디렉터리.
# 앱이 여기서 정책과 자기 머신 ID 파일을 받아간다. 그래서 공개일 수밖에 없다.
# 로컬에서 그냥 돌리면 이 저장소 안에 쓰이므로, 반드시 DIST 를 지정할 것.
DIST = Path(os.environ.get('LICENSE_DIST_DIR', ROOT / 'dist')).resolve()

REVOCATION_FILE = DIST / 'revoked.json'
LICENSES_DIR = DIST / 'licenses'
LICENSES_INDEX = LICENSES_DIR / 'index.json'
POLICY_FILE = DIST / 'license-policy.json'
...
def cmd_revoke(args: argparse.Namespace) -> None:
    ...
    print(action)
    print(f'{REVOCATION_FILE} 를 갱신했습니다. 배포 저장소에 커밋/푸시하면 적용됩니다.')
    print()
...
def cmd_policy(args: argparse.Namespace) -> None:
    ...
    print()
    print(f'{POLICY_FILE} 을 배포 저장소에 커밋/푸시해야 적용됩니다.')
    print()
